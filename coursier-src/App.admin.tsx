    ...
    <SafeAreaView style={st.loginRoot}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
        <View style={st.loginCard}>
          <View style={[st.loginBadge, { backgroundColor: colors.primaryDark }]}>
            <Ionicons name="shield-checkmark" size={28} color="#fff" />
          </View>
          <Text style={st.loginTitle}>Administration</Text>
          <Text style={st.loginSub}>Accès réservé — entrez votre PIN administrateur.</Text>

          <Text style={st.label}>PIN administrateur</Text>
          <TextInput
          ...
          <Pressable onPress={submit} style={[st.submit, { backgroundColor: colors.primaryDark }]}>
...
function AdminRoot() {
  ...
  if (role !== 'admin') return <AdminLoginScreen />;
  return (
