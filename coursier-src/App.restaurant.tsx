...
// Écran de connexion dédié aux restaurants.
// Aucun autre rôle n'est proposé — cette instance ne SERT QUE cette interface.
function RestaurantLoginScreen() {
  ...
  const submit = () => {
    setError('');
    const res = loginRestaurant(username.trim(), pin);
    ...
    <SafeAreaView style={st.loginRoot}>
      ...
      >
        <View style={st.loginCard}>
          <View style={st.loginBadge}>
            <Ionicons name="restaurant" size={28} color="#fff" />
          </View>
          <Text style={st.loginTitle}>Espace Restaurant</Text>
          ...
          <Pressable onPress={submit} style={st.submit}>
...
function RestaurantRoot() {
  const { role, ready } = useAuth();
  if (!ready) return null;
  if (role !== 'restaurant') return <RestaurantLoginScreen />;
  return (
...
const st = StyleSheet.create({
  ...
  error: { color: colors.danger ?? '#C0392B', fontSize: 12.5, marginTop: 10 },
  submit: {
